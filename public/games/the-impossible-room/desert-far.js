class DesertFar extends GameCore {
	...
	showChain(x, y, time) {
		...
		this.chain.changePosition(x * 10, this.chain.y, time);
	}
	...
	refresh(time, dt) {
		...
		if (this.startShrink) {
			...
			if (shrinkValue === -1) {
				...
				this.monkor.changePosition(390, 350, time);
				this.savedWalkArea = null;

				if (this.nextScene) {
					switch(this.nextScene) {
						case "N": 
							this.monkor.changePosition(390, 430, time);
							break;
						case "S":
							this.monkor.changePosition(390, 270, time);
							break;
						case "E":
							this.monkor.changePosition(100, 350, time);
							break;							
						case "W":
							this.monkor.changePosition(700, 350, time);
							break;							
					}
